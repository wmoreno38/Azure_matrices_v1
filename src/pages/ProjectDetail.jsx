...
/* ── Texto expandible ── */
function ExpandableText({ text, maxChars = 120, style = {} }) {
  const [expanded, setExpanded] = useState(false);
  if (!text) return <span style={{ color:'var(--text-light)', fontStyle:'italic', fontSize:'.78rem' }}>—</span>;
  const needsTruncation = text.length > maxChars;
  const displayed = expanded || !needsTruncation ? text : text.slice(0, maxChars) + '…';
  return (
    <div style={{ fontSize:'.8rem', lineHeight:1.45, whiteSpace:'pre-line', ...style }}>
      {displayed}
      {needsTruncation && (
        <button
          type="button"
          ...
          style={{
            ...
            marginLeft:5, padding:'1px 7px', fontSize:'.68rem', fontWeight:700,
            border:'1px solid var(--border)', borderRadius:99,
            background: expanded ? 'var(--orange-light)' : 'var(--bg)',
            color:'var(--orange)', cursor:'pointer', fontFamily:'var(--font)',
            transition:'all .15s', verticalAlign:'middle', whiteSpace:'nowrap',
          }}
        >
          {expanded ? '▲ menos' : '▼ más'}
        </button>
...
/* ── ComplianceInput — FIX stale closure con ref ── */
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:3}}>
      <div style={{display:'flex',gap:5}}>
        ...
        <button type="button" className={`btn btn-sm ${listening?'btn-danger':'btn-secondary'}`} onClick={toggleVoice} style={{flexShrink:0,padding:'6px 8px'}}>
...
function NormBox({ norm }) {
  ...
  return (
    <div style={{marginTop:8}}>
      <button type="button" onClick={()=>setOpen(o=>!o)} style={{display:'flex',alignItems:'center',gap:6,width:'100%',padding:'6px 10px',background:'var(--bg)',border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',cursor:'pointer',color:'var(--text-muted)',fontSize:'.7rem',fontWeight:700,fontFamily:'var(--font)',textTransform:'uppercase',letterSpacing:'.5px'}}>
      ...
      {open&&(
        <div style={{padding:14,borderTop:'1px solid var(--border)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,paddingTop:10,borderTop:'1px solid var(--border)',flexWrap:'wrap'}}>
            ...
            <div style={{flex:1}}/>
            {canApprove&&<><button type="button" className={`btn btn-sm ${localStatus==='aprobada'?'btn-primary':'btn-secondary'}`} onClick={()=>handleStatus('aprobada')}>✓ Aprobar</button><button type="button" className={`btn btn-sm ${localStatus==='rechazada'?'btn-danger':'btn-secondary'}`} onClick={()=>handleStatus('rechazada')}>✗ Rechazar</button><button type="button" className="btn btn-ghost btn-sm" onClick={()=>handleStatus('en_revision')}>↩ En revisión</button></>}
            {canDelete&&<button type="button" className="btn btn-danger btn-sm" onClick={()=>onDelete(ev.id)}>Eliminar</button>}
          </div>
        </div>
      )}
    </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {[
              ...
              <button type="button" key={i} onClick={()=>a.action==='export'?onExport():onNavigate(a.action==='approve'?'evidences':a.action)} disabled={a.action==='export'&&exporting}
          ...
          {canFinalize&&(
            <button type="button" onClick={()=>{if(finalizeError){alert(finalizeError);return;}onFinalize();}}
...
export default function ProjectDetail() {
  ...
  const handleAddEvidence=async e=>{
    ...
    try{
      const fd=await Promise.all(evForm.files.map(async f=>({name:f.name,type:f.type,data:await toBase64(f)})));
      await evidencesApi.create({projectId:id,controlId:evModal.id,...evForm,files:fd});
  ...
  return (
    <>
      <div className="page-header">
        <button type="button" className="btn btn-ghost btn-icon" onClick={()=>navigate('/projects')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 5l-7 7 7 7"/></svg></button>
        <div style={{flex:1}}>
          <h2>{project.name}</h2>
      ...
      <div className="page-body">
        <div className="tabs">
          {[{k:'overview',l:'Resumen'},{k:'controls',l:`Controles (${controls.length})`},{k:'evidences',l:`Evidencias (${evs.length})${pendingEvs>0?' 🔔':''}`}].map(v=><button type="button" key={v.k} className={`tab ${view===v.k?'active':''}`} onClick={()=>setView(v.k)}>{v.l}</button>)}
        </div>
          ...
          <>
            ...
            <div className="table-wrap card">
              <table style={{tableLayout:'fixed',width:'100%'}}>
                ...
                <thead>
                  <tr>
                    <th>Código</th><th>Causa base</th><th>Control base</th><th>RI</th><th>RR</th>
                    <th>Cumplimiento {canFill&&<span style={{color:'var(--orange)',fontSize:'.65rem'}}>🎙️</span>}</th>
                    <th>Evidencias</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredControls.map(c=>{
                    ...
                    return(
                      <tr key={c.id} style={{background:c.compliance?.trim()?'rgba(34,197,94,.03)':'transparent', verticalAlign:'top'}}>
                        <td style={{paddingTop:14}}>
                          <span className="chip" style={{fontFamily:'var(--mono)',fontWeight:700,color:'var(--orange)'}}>{c.code}</span>
                        </td>
                        <td>
                          <div style={{fontWeight:600,fontSize:'.8rem',marginBottom:4}}>{c.causaBase}</div>
                          {c.causasAsociadas&&<ExpandableText text={c.causasAsociadas} maxChars={100} style={{color:'var(--text-muted)',fontSize:'.75rem'}}/>}
                        </td>
                        <td>
                          <ExpandableText text={c.controlBase} maxChars={140}/>
                        ...
                        <td><ComplianceInput ctrl={c} onSave={saveCompliance} readOnly={!canFill}/></td>
                        <td style={{paddingTop:14}}>
                          <div style={{display:'flex',flexDirection:'column',gap:4}}>
                            {canAddEv&&<button type="button" className="btn btn-secondary btn-sm" onClick={()=>setEvModal(c)}>+ Evidencia</button>}
                            {evc>0&&<button type="button" className="btn btn-ghost btn-sm" style={{fontSize:'.72rem'}} onClick={()=>setView('evidences')}>{eap}/{evc} ✓</button>}
                          </div>
                        </td>
                      </tr>
                    );
        ...
        {view==='evidences'&&(
          <>
            ...
            {filteredEvs.length===0
              ?<div className="empty card" style={{padding:60}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p>No hay evidencias{filterEvStatus?' con este estado':'.'}</p></div>
      ...
      {evModal&&canAddEv&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setEvModal(null)}>
          <div className="modal modal-lg">
            <div className="modal-header"><span className="modal-title">Evidencia — <span style={{color:'var(--orange)',fontFamily:'var(--mono)'}}>{evModal.code}</span></span><button type="button" className="btn btn-ghost btn-icon" onClick={()=>setEvModal(null)}>✕</button></div>
      ...
      {finalModal&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setFinalModal(false)}>
          <div className="modal">
            <div className="modal-header"><span className="modal-title">Finalizar matriz</span><button type="button" className="btn btn-ghost btn-icon" onClick={()=>setFinalModal(false)}>✕</button></div>
            <div className="modal-body">
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" onClick={()=>setFinalModal(false)}>Cancelar</button>
              <button type="button" className="btn btn-primary" onClick={handleFinalize}>Confirmar y archivar</button>
            </div>
          </div>
        </div>
      )}
