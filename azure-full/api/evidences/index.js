...
export default async function handler(context, req) {
  ...
  if (req.method === 'POST' && !id) {
    ...
    if (!projectId || !controlId || !description)
      return jsonResponse(context, 400, { error: 'projectId, controlId y description son requeridos' });
    ...
    await query(
      ...
      [user.name, user.id, `Evidencia agregada: ${description} → ${ctrl?.code || ''}`, proj?.name || '']
    );

    return jsonResponse(context, { id: ev.id, success: true });
  }
  ...
  if (req.method === 'DELETE' && id) {
    ...
    if (files.length) {
      await deleteBlobs(files.map(f => f.storage_path));
    }
